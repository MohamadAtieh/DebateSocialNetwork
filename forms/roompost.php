...
$get_tags = mysqli_query($con, "SELECT * FROM `tag_table` WHERE LOWER(`tagname`) = LOWER('$tagname') && `category` = '$cat'") or die(mysqli_error($con));
...
if ($tag_rows > 0) {
    ...
    $insert_comment = mysqli_query($con, "INSERT INTO `comments_table`(`uid`, `tid`, `comment`, `side`, `replyto`, `reports`) 
                VALUES ('$uid','$tid','$comment','$side',IF(LENGTH('$reptyto')=0, NULL, '$reptyto'),0)");
    $cid = mysql_insert_id();

    mysqli_query($con, "UPDATE `user_stat` SET `posts` = CASE WHEN `posts` IS NULL THEN 1 ELSE (`posts` + 1) END 
            WHERE `uid` = $uid");
    ...
    foreach ($matches as $i) {
        ...
        $get_user = mysqli_query($con, "SELECT * FROM `user_table` WHERE `username` = '$target_mention'");
        $user_arr = mysqli_fetch_array($get_user, MYSQLI_BOTH);
        if ($user_arr > 0) {
            $id = $user_arr['uid'];
            if ($id != $uid) {
          ...
          WHERE `uid` = $id");
            ...
            VALUES ('$id','$uid','$not','$type','$cid')") or die(mysqli_error($con));
