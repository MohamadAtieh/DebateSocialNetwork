...
if (!isset($_GET["cid"]) || trim($_GET["cid"]) == "")
    ...
    if ($exist) { //if the user has a previous activity, update
        ...
        if ($like === '2') {
            ...
            if ($b == FALSE) {
                mysqli_query($con, "UPDATE `user_stat` SET `dislikes` = CASE WHEN `dislikes` = 1 THEN NULL ELSE (`dislikes` - 1) END 
            WHERE `uid` = $uid");
                mysqli_query($con, "UPDATE `user_stat` SET `dislikes_received` = CASE WHEN `dislikes_received` = 1 THEN NULL ELSE (`dislikes_received` - 1) END 
            WHERE `uid` = $co");
            }
            ...
            mysqli_query($con, "UPDATE `activity_table` SET `LD`= 2 WHERE `cid` = $cid AND `uid` = $uid");
...
function trigger($cid, $belongs, $sender, $receiver) {
    if ($belongs == FALSE) {
        //update stats
        mysqli_query($con, "UPDATE `user_stat` SET `dislikes` = CASE WHEN `dislikes` IS NULL THEN 1 ELSE (`dislikes` + 1) END 
            WHERE `uid` = $sender");
        mysqli_query($con, "UPDATE `user_stat` SET `dislikes_received` = CASE WHEN `dislikes_received` IS NULL THEN 1 ELSE (`dislikes_received` + 1) END 
            WHERE `uid` = $receiver");
