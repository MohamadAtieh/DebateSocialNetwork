...
if (mysqli_num_rows($query) > 0) {
    while ($act = mysqli_fetch_array($query,MYSQLI_BOTH)) {
        $act_id = $act['act_id'];
        ...
        $sender_query = mysqli_query($con, "SELECT * FROM `user_table` WHERE `uid` = $sender_id");
        $sender_user = mysqli_fetch_array($sender_query,MYSQLI_BOTH);

        $target_query = mysqli_query($con, "SELECT * FROM `user_table` WHERE `uid` = $target_id");
        $target_user = mysqli_fetch_array($target_query,MYSQLI_BOTH);

        if ($sender_user > 0 && $target_user > 0) {
            if ($type != 'follow') {
                //comment based activity
                $room_query = mysqli_query($con, "SELECT t.tagname, t.category, c.side, c.comment FROM tag_table t, comments_table c WHERE c.cid = $target_object
                        AND c.tid = t.tid");
