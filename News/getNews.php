...
$name = $_GET['name'];
$get_news = "SELECT C.tid AS tid,tagname,category , "
        . "COUNT(C.cid) AS comments, COUNT(DISTINCT C.uid)AS users FROM comments_table "
        . "AS C INNER JOIN tag_table ON C.tid = tag_table.tid WHERE category = '$name' "
        . "GROUP BY tagname ORDER BY comments DESC limit 2";
$get_news = mysqli_query($con, $get_news);
$json = array();
$percentage = array();
