...
$get_user = mysqli_query($con, "SELECT * FROM info_table WHERE uid='$id'");
$user = mysqli_fetch_array($get_user, MYSQLI_BOTH);
#####################################

$fid = $_SESSION['id'];

######################################
$get_following = mysqli_query($con, "SELECT * FROM followers_table WHERE uid ='$id' AND follower_id='$fid' ");
$following = mysqli_fetch_array($get_following, MYSQLI_BOTH);
$follow = 'Follow';
if ($following != NULL) {
    $follow = "Following";
}
