...
if (isset($_GET["tag"]) && isset($_GET["cat"])) {
    ...
    $tag = '#' . $_GET["tag"];
    $uid = $_SESSION['id'];
    $search_tags = mysqli_query($con, "SELECT * FROM tag_table  WHERE LOWER(`tagname`) = LOWER('$tag') && `category` = '$category'");
    $target_tag = mysqli_fetch_array($search_tags, MYSQLI_BOTH);
    $tid = $target_tag['tid'];
    $started = date('d M y', strtotime($target_tag['Date']));
    $owner_id = $target_tag['uid'];
    $owner_query = mysqli_query($con, "SELECT * FROM user_table WHERE uid=$owner_id");
    $owner = mysqli_fetch_array($owner_query, MYSQLI_BOTH);
                ...
                $.ajax({                                      
                    ...
                    data: "", async:false,dataType: 'json',  success: function(data)        
                    {
                        ...
                        for (var i in data)
                        {
                            ...
                            if(session_id == uid){ //if comment's user is current user -> require_once delete
